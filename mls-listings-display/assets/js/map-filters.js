...
const MLD_Filters = {
     ...
     * @returns {object} The default filter object.
     */
     ...
     * @param {boolean} isForCountOrOptions - If true, returns a temporary state without saving it.
     ...
     * @returns {object} The combined filter object.
     */
    ...
    /**
     * Removes a keyword filter tag and refreshes the map.
     */
    removeKeywordFilter: function(type, value) { 
        const app = MLD_Map_App;
        if (app.keywordFilters[type]) { 
            app.keywordFilters[type].delete(value); 
            if (app.keywordFilters[type].size === 0) delete app.keywordFilters[type]; 
        } 
        this.renderFilterTags(); 
        MLD_Core.updateUrlHash();
        MLD_API.refreshMapListings(true); 
    ...
    /**
     * Renders the active keyword filter tags below the search bar.
     */
    renderFilterTags: function() { 
        ...
        $container.empty(); 
        for (const type in MLD_Map_App.keywordFilters) { 
            MLD_Map_App.keywordFilters[type].forEach(value => { 
                const $tag = $(`<div class="bme-filter-tag" data-type="${type}" data-value="${value}">${value} <span class="bme-filter-tag-remove">&times;</span></div>`); 
                $tag.find('.bme-filter-tag-remove').on('click', () => this.removeKeywordFilter(type, value)); 
